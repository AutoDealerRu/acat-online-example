...
<body>
<?php
$previousGroupUrl = $previousGroup ? "/{$hrefPrefix}{$previousGroup->type}/{$previousGroup->mark}/{$previousGroup->country_short_name}/{$previousGroup->model}/{$previousGroup->year}/{$previousGroup->catalog_code}/{$previousGroup->dir}/{$previousGroup->group_type}/{$previousGroup->group}/{$previousGroup->subgroup}/{$previousGroup->detail}" : null;
$nextGroupUrl = $nextGroup ? "/{$hrefPrefix}{$nextGroup->type}/{$nextGroup->mark}/{$nextGroup->country_short_name}/{$nextGroup->model}/{$nextGroup->year}/{$nextGroup->catalog_code}/{$nextGroup->dir}/{$nextGroup->group_type}/{$nextGroup->group}/{$nextGroup->subgroup}/{$nextGroup->detail}" : null;
$title = $breadcrumbs[6]->name;
?>

<div class="list-group" style='text-align: center; position: relative;'>
    <?php if ($previousGroup) { ?>
        <a class="group-prev" href="<?php echo $previousGroupUrl ?>" title="<?php echo $previousGroup->name ?>">
            <span class="list-group-prev"></span>
        </a>
    <?php } ?>
    <h1 class="title" style="margin: 0 50px; display: inline-block;"><?php echo $title ?></h1>
    <?php if ($nextGroup) { ?>
        <a class="group-next" href="<?php echo $nextGroupUrl ?>" title="<?php echo $nextGroup->name ?>">
...
<div class="image-area" onselectstart="return false;">
    ...
    foreach ($numbers as $item) {
        if (property_exists($item,'coordinate')) {
            foreach ($item->coordinate as $coordinate) {
                $index = ($item->label ? $item->label : ($item->number_formatted ? $item->number_formatted : $item->full_name));
                ...
                if (!in_array($coordinateIndex, $added)) {
                    ...
                    $labels[] = json_decode(json_encode([
                        'index' => $index,
                        'title' => "{$item->name} ({$item->number_formatted})",
        ...
        <div class="main-image-area">
            ...
            <div class="main-image imageArea">
                <span class="imageLayout">
                    <img src="<?php echo $subgroup->illustrate ?>">
...
<table class="table imageArea-related active">
    ...
    <?php $ind = ($item->label ? $item->label : ($item->number_formatted ? $item->number_formatted : $item->full_name)) ?>
        <tr class="table-row bottom-line to-image" data-index="<?php echo $ind ?>">
            <td class="table-cell"><?php echo $item->label ?></td>
            <td class="table-cell number-info-cell">
                <?php if (property_exists($item, 'number_formatted')) { ?>
                    ...
                    <div class="modal-number-info">
                        ...
                        <div class="number-info">
                            ...
                            <?php if (property_exists($item,'date_start') || property_exists($item,'date_end')) { ?>
                            ...
                            <?php if (property_exists($item,'note')) { ?>
                                <div class="item-info-params"><?php echo "Примечание: {$item->note}" ?></div>
                            <?php } ?>
                            <?php if (property_exists($item,'model_info')) { ?>
                                <div class="number-info-params"><?php echo "Данные по модели: {$item->model_info}" ?></div>
                            <?php } ?>
                            <?php if (property_exists($item,'amount')) { ?>
                                <div class="number-info-params"><?php echo "Количество: {$item->amount}" ?></div>
                            <?php } ?>
                        </div>
                    </div>
                <?php } ?>
            </td>
            <td class="table-cell"><?php echo $item->number_formatted ?></td>
