...
<div class="list-group" style='text-align: center; position: relative;'>
    <?php if ($previousGroup) { ?>
        <a class="group-prev" href="<?php echo $previousGroupUrl ?>" title="<?php echo "{$previousGroup->service_name} {$previousGroup->subservice_name}" ?>">
            <span class="list-group-prev"></span>
        </a>
    <?php } ?>
    <h1 class="title" style="margin: 0 50px; display: inline-block;"><?php echo $title ?></h1>
    <?php if ($nextGroup) { ?>
        <a class="group-next" href="<?php echo $nextGroupUrl ?>" title="<?php echo "{$nextGroup->service_name} {$nextGroup->subservice_name}" ?>">
...
<table class="table">
    ...
    <tr class="table-row bottom-line a2s-numbers_header">
        <td class="table-cell">Изображения</td>
        ...
        <tr class="table-row bottom-line to-image" data-index="<?php echo $ind ?>">
            <td class="table-cell img-cell">
                <?php if (property_exists($item, 'img')) { ?>
                    <img src="<?php echo $item->img ?>" onError="this.src='https://212709.selcdn.ru/autocatalog-online/public/images/avtodiler.png'">
                <?php } else { ?>
                ...
                <?php if (property_exists($item, 'number_formatted')) { ?>
                    ...
                    <div class="modal-number-info">
                        ...
                        <div class="number-info">
                            ...
                            <?php } ?>
                            <?php if (property_exists($item,'pr_number')) { ?>
                                <div class="number-info-params"><?php echo "PR номера: {$item->pr_number}" ?></div>
                            <?php } ?>
