...
<div class="image-tabs">
    <div class="image-tabs-nav">
        <?php if ($images && is_array($images) && count($images) > 1) foreach ($images as $index => $image) { ?>
            <a href="<?php echo "/{$hrefPrefix}{$image->type}/{$image->mark}/{$image->country}/{$image->aggregation}/{$image->model}/{$image->catalog}/{$image->group}/{$image->subgroup}".(property_exists($image,'sa_id') ? "/{$image->sa_id}/{$image->stroke}" : '')."/{$image->position}"?>">
                 <span class="image-tab-nav href-tab <?php echo property_exists($image,'current') && $image->current ? 'active' : ''?>">
...
<table class="table imageArea-related active">
    ...
    <?php foreach ($numbers as $key => $number) { ?>
        <tr class="table-row bottom-line to-image" data-index="<?php echo $number->label ?>">
            ...
            <td class="table-cell number-info-cell" data-number-info="<?php echo $key?>">
                ...
                <div class="modal-number-info">
                    ...
                    <div class="number-info">
                        <?php if (property_exists($number,'bad_number') && strlen($number->bad_number) > 0) { ?>
                            <div class="number-info-params" style="color: red;">
                                - <?php echo $number->bad_number ?>
                                <span style="font-size: 10px;padding-left: 20px;"> ("не применяется с" / "не подходит" / "плохой")</span>
                            </div>
                        <?php } ?>
                        <?php if (property_exists($number,'replaced_on') && count($number->replaced_on) > 0) { ?>
                            <div class="number-info-params" style="color: blue;">Заменен на: <?php echo implode(', ', $number->replaced_on) ?></div>
                        <?php } ?>
                        <?php if (property_exists($number,'same_numbers') && count($number->same_numbers) > 0) { ?>
                            <div class="number-info-params" style="color: green;">На выбор с: <?php echo implode(', ', $number->same_numbers) ?></div>
                        <?php } ?>
                        <?php if (property_exists($number,'count') && $number->count) { ?>
                            <div class="number-info-params">Количество: <?php echo $number->count ?></div>
                        <?php } ?>
                        <?php if (property_exists($number,'footnotes') && count($number->footnotes) > 0) { ?>
                            <div class="number-info-count">
                                <p>Дополнительно:</p>
                                <div class="numbers_count" style="font-size: 14px;">
                                    <?php foreach ($number->footnotes as $note) { ?>
                                        <p style="line-height: 15px;"><?php echo $note ?></p>
                                    <?php } ?>
                                </div>
                            </div>
                        <?php } ?>
            ...
            <td class="table-cell"><?php echo $number->name.(property_exists($number,'description') ? $number->description : '') ?></td>
        </tr>
    <?php } ?>
    </tbody>
</table>
