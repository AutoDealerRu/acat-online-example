...
<table class="table imageArea-related active">
    ...
    foreach ($numberGroup->parts as $index2 => $number) {
        ...
        <tr class="table-row bottom-line to-image" data-index="<?php echo $ind ?>">
            ...
            <td class="table-cell">
                <?php $name = $number->name ? $number->name . ($numberGroup->name && $numberGroup->name !== $number->name && $numberGroup->name !== $breadcrumbs[5]->name ? ' ('.$numberGroup->name.')':'') : $numberGroup->name; ?>
                <?php if ($number->groupId) { ?>
                    <a href="./<?php echo $number->groupId?>"><?php echo $name?></a>
                <?php } else { ?>
