...
<table class="table imageArea-related active table-tab active">
    <thead class="table-head">
        <tr class="table-row bottom-line">
            <td class="table-cell">Группа</td>
            <td class="table-cell">Деталь</td>
        </tr>
    </thead>
    <tbody class="table-body">
    <?php foreach ($unit->groups as $group) {?>
        <tr class="table-row bottom-line to-image" data-index="<?php echo $group->name ? $group->name : '' ?>">
            <td class="table-cell" style="width:25%;"><?php echo $group->name ?></td>
            <td class="table-cell" style="vertical-align: top; padding: 0;">
                <table class="table table-child">
                    ...
                    <?php foreach ($group->subgroups as $subgroup) {
                        $url = "/{$hrefPrefix}{$breadcrumbs[1]->url}/{$unit->mark_short_name}/{$unit->model_short_name}/{$unit->modification_short_name}/{$unit->category_short_name}/{$subgroup->short_name}";?>
                        <tr class="table-row bottom-line">
                            <td class="table-cell" style="padding: 10px;">
                                <a href="<?php echo $url ?>"><?php echo $subgroup->name ?></a>
                            </td>
                        </tr>
                    <?php } ?>
                    </tbody>
